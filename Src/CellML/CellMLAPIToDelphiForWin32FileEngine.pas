//==============================================================================
// Engine class for exporting a CellML API object to a Delphi for Win32 file
//
// by Alan Garny
...
Unit CellMLAPIToDelphiForWin32FileEngine;
...
Type
   TCellMLAPIToDelphiForWin32FileEngine = Class(TCellMLAPIExportEngine)
      Protected
...
Constructor TCellMLAPIToDelphiForWin32FileEngine.Create(Const aCellMLModel: TCellMLModel;
...
Procedure TCellMLAPIToDelphiForWin32FileEngine.Execute;
Var
...
Begin
   ...
   Output(0, StatementCommentTag+StringOfChar('-', 78-Length(CellMLIndent[0])));
{$IFDEF COR_SPECIFIC}
   Output(0, StatementCommentTag+' Conversion from '+CELLML_VERSION+' to Delphi for Win32 was done using '+COR_NAME+' ('+COR_VERSION+')');
...
{$ENDIF}
   ...
   ClassNameStr := 'T'+ChangeFileExt(ExtractFileName(FileName[0]), '');

   Output(0, ClassNameStr+' = Class');
   ...
   Output(0, 'Procedure Compute(Const '+tStr+': Double);');
   ...
   If (Not AtEnd(BinarySearch(CellMLModel.RequiredFunctions, ['fact']))) Then Begin
      ...
      Output(0, StatementCommentTag+StringOfChar('-', 78-Length(CellMLIndent[0])));
      Output(0);
      Output(0, 'Function '+ClassNameStr+'.Fact(N: Double): Double;');
      Output(0, 'Begin');
   ...
   Output(0, StatementCommentTag+StringOfChar('-', 78-Length(CellMLIndent[0])));
   Output(0);
   Output(0, 'Procedure '+ClassNameStr+'.Init;');
   Output(0, 'Begin');
   ...
   Output(0, StatementCommentTag+StringOfChar('-', 78-Length(CellMLIndent[0])));
   Output(0);
   Output(0, 'Procedure '+ClassNameStr+'.Compute(Const '+tStr+': Double);');
   Output(0, 'Begin');
...
Function TCellMLAPIToDelphiForWin32FileEngine.AddFunc(Const aCellMLComponent: String;
