...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jcl.svn.sourceforge.net:443/svnroot/jcl/tags/JCL-2.1-Build3536/jcl/source/windows/JclAppInst.pas $';
...
//---GRY--- START
{$IFDEF COR_SPECIFIC}
  ProcID: DWORD;   // To keep track of the process ID
{$ENDIF}
//---GRY--- END
...
const
  ClassNameOfTMainForm = 'TMainForm';
var
  MainWnd: TMainWnd;

  function EnumWinProc(Wnd: THandle; Param: PMainWnd): BOOL; stdcall;
  var
    PID: DWORD;
    C: array [0..Length(ClassNameOfTMainForm) + 1] of Char;
  begin
    GetWindowThreadProcessId(Wnd, @PID);
    if (PID = Param^.ProcessID) and (GetClassName(Wnd, C, SizeOf(C)) > 0) and
      (C = ClassNameOfTMainForm) then
    begin
      Result := False;
      Param^.Wnd := Wnd;
    end
...
//---GRY--- START
{$IFDEF COR_SPECIFIC}
  // Note: the idea is here to restore the main form, should it be minised, this
  //       before setting the application to the foreground... This is a bit of
  //       a pain, but that's because of Delphi and its use of a hidden window,
  //       which Vista doesn't really like... hence we had to use a hack within
  //       COR, which now has an effect on TJclAppInstances... Thank god,
  //       everything works fine also under Windows XP...
  MainWnd.ProcessID := ProcID;
  MainWnd.Wnd := 0;
  EnumWindows(@EnumWinProc, LPARAM(@MainWnd));

  if IsIconic(MainWnd.Wnd) then
    SendMessage(MainWnd.Wnd, WM_SYSCOMMAND, SC_RESTORE, 0);
{$ELSE}
//---GRY--- END
...
//---GRY--- START
{$IFDEF COR_SPECIFIC}
  ProcID := GetProcessIDs(Index);   // Keep track of the process ID
{$ENDIF}
//---GRY--- END
