...
//---GRY--- START
    Recipients.Add('COR@physiol.ox.ac.uk');
    Subject := 'COR - Bug Report';
    Body := '**********************************************************'+_CRLF_+
            '*                                                        *'+_CRLF_+
            '*                 PLEASE PROVIDE US WITH                 *'+_CRLF_+
            '*  WORKING FILE(S) AND INSTRUCTION(S) WHENEVER POSSIBLE  *'+_CRLF_+
            '*                                                        *'+_CRLF_+
            '**********************************************************'+_CRLF_+
            _CRLF_+
            AnsiString(ReportAsText);
//---GRY--- END
    SaveTaskWindows;
    try
      Send(True);
...
//---GRY--- START
  ...
  DetailsMemo.DoubleBuffered := True;
//---GRY--- END
end;
...
begin
  ...
  begin
//---GRY--- START
    Top := Top-(FFullHeight-ClientHeight) Div 2;
//---GRY--- END
