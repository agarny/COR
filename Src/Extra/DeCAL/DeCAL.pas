...
{$ENDIF}
  ...
  DListNode = class(DBaseClass)
    ...
    destructor Destroy(const aParent: Boolean = false); reintroduce;
...
destructor DTreeNode.Destroy(const aParent: Boolean);
begin
  if (not aParent) then begin
     {$IFDEF NOTDEFINED}
...
destructor DListNode.Destroy(const aParent: Boolean);
begin
  if (not aParent) then
     ClearDObject(obj);
