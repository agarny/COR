...
// Note: add a handler to avoid the flickering that occurs under Windows Vista.
//       Note that this doesn't affect other versions of Windows...
//---GRY--- END
  protected
    procedure CreateParams(var Params: TCreateParams); override;
    procedure CreateWnd; override;
...
begin
  ...
  Height := 193;
  TabStop := True;
  ControlStyle := [csAcceptsControls, csDoubleClicks, csPannable];
...
begin
  inherited Create(AOwner);
  ControlStyle := [csDoubleClicks, csOpaque, csPannable];
  FPages := TList.Create;
end;
...
var
  NextSheet: TTabSheet;
begin
//---GRY--- There is a problem with the original VCL code, which is about the
//          next page to be activated. Basically, the original code selects the
//          NEXT page, while the Delphi IDE selects the PREVIOUS page except if
//          we want to remove the first page, in which case it will select the
//          NEXT page. So, that's what we are doing here...
//          A special case, however, is when we have so many tabs that we get 
//          those two buttons at the top right of the control to allow one to
//          scroll the tabs. If one keeps closing the last page until the page,
//          which tab is the last visible one on the component, then once that
//          page is closed, no tab is visible... To prevent that from happening
//          we have to activate the next page, before releasing the current
//          one...
{---GRY---
  NextSheet := FindNextPage(Page, True, not (csDesigning in ComponentState));
}
//---GRY--- BEGIN
NextSheet := FindNextPage(Page, Page.PageIndex = 0, Not (csDesigning In ComponentState));
//---GRY--- END
...
//---GRY--- END
          ...
          try
            ...
            begin
              ...
              FTrackPos.X := TSmallPoint(MsgPos).X;
              FTrackPos.Y := TSmallPoint(MsgPos).Y;
...
begin
  inherited Create(AOwner);
  ControlStyle := ControlStyle - [csCaptureMouse] + [csDisplayDragImage, csReflector, csPannable];
  Width := 121;
  Height := 97;
  TabStop := True;
  ParentColor := False;
...
procedure TCustomTreeView.EditWndProc(var Message: TMessage);
  ...
  begin
    ...
    try
      SendMessage(FEditHandle, WM_PRINTCLIENT, WPARAM(MemDC), PRF_CLIENT);
        ...
        begin
          ...
          if RightClickSelect then
            ...
            begin
              ...
              Perform(WM_CONTEXTMENU, WPARAM(Handle), LPARAM(PointToSmallPoint(MousePos)));
              FRClickNode := nil;
            end
          else
            // Win95/98 eat WM_CONTEXTMENU when posted to the message queue  
            PostMessage(Handle, CN_BASE+WM_CONTEXTMENU, WPARAM(Handle), LPARAM(PointToSmallPoint(MousePos)));
...
begin
  ...
  begin
    Style := Style or OrientationStyle[FOrientation] or
      LTickStyles or ATickMarks[FTickMarks] or TBS_FIXEDLENGTH;
    WindowClass.style := WindowClass.style and not (CS_HREDRAW or CS_VREDRAW) or
      CS_DBLCLKS;
...
begin
  ...
  FMax := 100;
//---GRY--- BEGIN
Limit16_Max_Min := Limit16/(FMax-FMin);
//---GRY--- END
  FStep := 10;
...
//---GRY--- Create a progress bar, which minimum and maximum remain constant (at
//          least in Windows' eyes, not internally). The reasoning behind this
//          is that when changing the position of the progress bar under Windows
//          Vista (Aero or Basic mode), it won't go straight to the position,
//          but will gradually go to it. The side effect is the progress bar's
//          position is updated too quickly, we may 'close' the progress bar it
//          has reached its final position (!!). The only way to get Windows
//          Vista to go straight to the required position is to do what is done
//          in SetPosition, i.e. go to the required position and then go to the
//          required position minus one. Yes, by going backwards, we effectively
//          go straight to the required position minus one. The minus one is not
//          too much of an issue, since the width of the progress bar is 65536,
//          so even on a high resolution screen, the difference won't be
//          noticeable...
//---GRY--- BEGIN
If (F32BitMode) Then
   SendMessage(Handle, PBM_SETRANGE32, 0, Limit16)
Else
   SendMessage(Handle, PBM_SETRANGE, 0, MakeLong(0, Limit16));

Val := Round(Limit16_Max_Min*(FPosition-FMin));
...
If ((Value <> FPosition) And (Value >= FMin) And (Value <= FMax)) Then Begin
   If (HandleAllocated) Then Begin
      Val := Round(Limit16_Max_Min*(Value-FMin));
...
//---GRY--- BEGIN
Procedure TProgressBar.WMEraseBkgnd(Var aMsg: TWMEraseBkgnd);
Begin          
   DefaultHandler(aMsg);
End;
//---GRY--- END
...
begin
  ...
  if Win32MajorVersion < 5 then
    Perform(WM_CONTEXTMENU, WPARAM(Handle), LPARAM(PointToSmallPoint(
...
begin
  ...
  begin
    ...
    SendMessage(Handle, UDM_SETBUDDY, WPARAM(FAssociate.Handle), 0);
  end;
  Width := OrigWidth;
...
begin
  ...
  begin
    ...
    begin
      ...
      SendMessage(Handle, UDM_SETBUDDY, WPARAM(Value.Handle), 0);
    end;
    FAssociate := Value;
...
begin
  if (FWidth = 0) or ((not AutoSize) and TListColumns(Collection).Owner.HandleAllocated and
     (TListColumns(Collection).Owner.ViewStyle = vsReport) and (FWidth <> LVSCW_AUTOSIZE) and
     (TListColumns(Collection).Owner.ValidHeaderHandle)) then
...
procedure TCustomListView.HeaderWndProc(var Message: TMessage);
  ...
  begin
    PaintBuffer := BeginBufferedPaint(DC, Rect, BPBF_TOPDOWNDIB, nil, MemDC);
    try
      SendMessage(FHeaderHandle, WM_PRINTCLIENT, WPARAM(MemDC), PRF_CLIENT);
      BufferedPaintMakeOpaque(PaintBuffer, @Rect);
    finally
      EndBufferedPaint(PaintBuffer, True);
    end;
      ...
      LVN_ENDLABELEDIT:
        with PLVDispInfo(NMHdr)^ do
          if (item.pszText <> nil) and (item.IItem <> -1) then
            Edit(item);
      LVN_COLUMNCLICK:
        ColClick(GetColumnFromTag(PNMListView(NMHdr)^.iSubItem));
      LVN_INSERTITEM:
...
var
  Form: TCustomForm;
begin
  if DoubleBuffered and DwmCompositionEnabled and (FSaveSelectedIndex = -1) then
  begin
    Form := GetParentForm(Self);
    if (Form <> nil) and Form.GlassFrame.FrameExtended and
      Form.GlassFrame.IntersectsControl(Self) then
    begin
      FSaveSelectedIndex := 0; // Used as flag to preserve unit signature
      PostMessage(FEditHandle, CM_BUFFEREDPRINTCLIENT, 0, 0);
    end;
  end
  else
...
begin
  ...
  begin
    ...
    begin
      if FindResource(MainInstance, PChar(CommonAVIId[FCommonAVI]), ResType) <> 0 then
        Result := MainInstance
      else if FindResource(HInstance, PChar(CommonAVIId[FCommonAVI]), ResType) <> 0 then
        Result := HInstance
...
begin
  ...
  Result := Perform(ACM_OPEN, WPARAM(GetActualResHandle), GetActualResId) <> 0;
      ...
      begin
        ...
        begin
          if (Control is TToolButton) and
            ...
            if Perform(TB_GETRECT, I, Longint(@LRect)) <> 0 then
...
{$WARNINGS OFF}
//---GRY--- END
            if nmcd.dwItemSpec < FButtons.Count then
//---GRY--- BEGIN
{$WARNINGS ON}
//---GRY--- END
...
var
  ...
  begin
    ...
    SendMessage(Handle, WM_NOTIFY, WPARAM(Handle), Longint(@Msg));
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawText(FBitmap.Canvas.Handle, PChar(Str), Length(Str), R,
...
begin
  ...
  begin
    Perform(WM_SETFONT, WPARAM(Font.Handle), 0);
    if not (csLoading in ComponentState) then
      RecreateWnd;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Result then
      begin
        TToolButton(NewButton).FToolbar := Self;
        if FButtons.IndexOf(NewButton) = -1 then
        begin
          FButtons.Insert(NMToolbar^.iItem, NewButton);
          if Assigned(FOnCustomizeAdded) then
            FOnCustomizeAdded(Self, TToolButton(NewButton));
        end;
      end;
    end;
  end
...
begin
  ...
  if (FControl <> nil) then
    Perform(PGM_SETCHILD, 0, LPARAM(FControl.Handle));
...
begin
  ...
  begin
    ...
    SendMessage(Handle, PGM_SETCHILD, 0, LPARAM(FControl.Handle));
  end
  else
...
begin
  ...
  begin
    ...
    Images.FreeNotification(Self);
    if HandleAllocated then
      PostMessage(Handle, CBEM_SETIMAGELIST, 0, LPARAM(Images.Handle));
  end
  else
  begin
    if HandleAllocated then
...
begin
  if HandleAllocated then
    Perform(CBEM_SETIMAGELIST, 0, LPARAM(TCustomImageList(Sender).Handle));
end;
